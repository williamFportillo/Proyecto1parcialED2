...
void archivos::Importar(file_entry array[] , bitmap *bitmap){
    file_entry tmp;
    int bloquesAOcupar=0;

    cout<<"Nombre del documento a importar:";
    cin>>tmp.file_name;
    ifstream lectura(tmp.file_name, ios::in | ios::binary);
    if(lectura) {
        lectura.seekg(0 , ios::end);
        sizeArchivos=lectura.tellg();
        if(sizeArchivos<= (cantidad_bloques*4096)){
            int actual = this->posActual;
            tmp.padre=actual;
            tmp.file_type='A';
...
void archivos::apagarBits(file_entry array[], string name, bitmap* bmap, int bloques) {
    for (int i = 0; i <=bloques ; i++) {
        bmap->setOf(bmap->DBlock,i);
    }
}

void archivos::Exportar(file_entry *array, bitmap *bitmap) {
    file_entry tmp;
    char archivo[30];
    cout<<"Nombre del documento a exportar: ";
    cin>>tmp.file_name;
    cout<<"Nuevo nombre: ";
    cin>>archivo;
    ofstream escribir(archivo, ios::out|ios::binary);
